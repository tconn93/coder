...
async function runAgentCLI(prompt, options, verbose) {
    ...
    for await (const event of orchestrator.run(prompt, options)) {
        switch (event.type) {
            ...
            case 'tool_call': {
                ...
                if (inp?.file_path)
                    detail = ` ${chalk.gray(String(inp.file_path))}`;
                else if (inp?.path)
                    detail = ` ${chalk.gray(String(inp.path))}`;
                else if (inp?.command)
                    detail = ` ${chalk.gray(String(inp.command).slice(0, 60))}`;
                else if (inp?.pattern)
                    detail = ` ${chalk.gray(String(inp.pattern))}`;
                else if (inp?.query)
                    detail = ` ${chalk.gray(String(inp.query).slice(0, 60))}`;
...
program
    ...
    .option('--permission-mode <mode>', 'Permission mode: default|acceptEdits|bypassPermissions|plan', 'acceptEdits')
    ...
    const permissionMode = String(opts['permissionMode'] || 'acceptEdits');
    ...
    if (opts['resume']) {
        ...
        if (!prompt) {
            ...
            rl.question(chalk.cyan('Resume prompt > '), async (resumePrompt) => {
                ...
                for await (const event of orchestrator.resume(resumeId, resumePrompt, agentOptions)) {
            ...
            const orchestrator = new AgentOrchestrator(workdir);
            for await (const event of orchestrator.resume(resumeId, prompt, agentOptions)) {
                if (event.type === 'text')
                    process.stdout.write(event.data);
                if (event.type === 'done')
                    break;
