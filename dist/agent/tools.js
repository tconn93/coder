...
// ---------------------------------------------------------------------------
export function createBaseTools(workdir, todoTracker) {
    return {
        read_file: tool({
        ...
        edit_file: tool({
            description: 'Edit a file by replacing exact text. old_string must match exactly including whitespace.',
            inputSchema: z.object({
        ...
        bash: tool({
            description: 'Execute a shell command in the working directory. Use for git, npm, running tests, build commands, etc.',
            inputSchema: z.object({
        ...
        grep: tool({
            ...
            execute: async (input) => {
                ...
                try {
                    ...
                    const e = err;
                    if (e.code === 1)
                        return 'No matches found'; // grep exits 1 on no match
                    return `Error: ${e.message}`;
                }
            },
        }),
        todo_write: tool({
            description: 'Create or update a todo list to track multi-step task progress. Use for tasks with 3+ distinct steps.',
            inputSchema: z.object({
...
// ---------------------------------------------------------------------------
export function createTools(workdir, todoTracker, provider) {
...
- doc-writer: Write README, API docs, inline documentation`,
        inputSchema: z.object({
            ...
            prompt: z
                .string()
                .describe('Detailed task description including relevant file paths and context'),
        }),
        execute: async (input) => {
            const { name, prompt } = input;
            const def = SUBAGENT_DEFS[name];
            if (!def)
                return `Unknown subagent: ${name}`;
            const subTodo = new TodoTracker();
            const allBase = createBaseTools(workdir, subTodo);
            const subTools = Object.fromEntries(def.tools
                .filter((t) => t in allBase)
                .map((t) => [t, allBase[t]]));
            try {
