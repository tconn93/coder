...
export async function* runAgentLoop(options) {
    ...
    const pendingTodoEvents = [];
    todoTracker.on('change', (todos) => {
    ...
    try {
        ...
        for await (const part of result.fullStream) {
            ...
            switch (part.type) {
                ...
                case 'error':
                    yield {
                        type: 'error',
                        data: { message: String(part.error), code: 'STREAM_ERROR' },
                    };
                    break;
                default:
        ...
        const response = await Promise.resolve(result.response).catch(() => ({ messages: [] }));
        const allMessages = [
            ...messages,
            ...response.messages,
        ];
